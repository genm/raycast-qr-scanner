...
Opening a recognized `http`, `https`, `mailto`, or `tel` payload is always a separate user action. Valid `FIDO:/` hybrid authentication payloads use an explicit Request Passkey action, which hands the URI to a registered FIDO app. If no handler is installed, the action reports the failure and advises scanning the original QR code with a compatible passkey device. Other URI schemes are displayed as text.
